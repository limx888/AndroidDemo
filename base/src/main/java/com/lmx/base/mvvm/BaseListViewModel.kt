...
import com.lmx.base.http.exception.NetWorkException
import com.lmx.base.http.exception.ResultException
import com.lmx.base.http.exception.ReturnCodeException
import com.lmx.base.http.exception.ReturnCodeNullException
import com.lmx.base.http.interceptor.IReturnCodeErrorInterceptor
import com.lmx.base.utils.NetworkUtils
...
abstract class BaseListViewModel<API> : ViewModel(), LifecycleObserver {
    ...
    private suspend fun <T> executeResponse(
        ...
        coroutineScope {
            //多baseurl
            if (BaseConfig.getMoreBaseUrl() && currentDomainName != BaseConfig.DOMAIN_NAME) {
                //获取当前baseurl对应的成功码
                val retSuccessList =
                    BaseConfig.getRetSuccessMap()?.get(currentDomainName)
                //当前对应的baseurl对应的code
                if (retSuccessList != null) {
                    //状态码正确
                    if (retSuccessList.contains(response.getBaseCode())) {
                        //数据为空，或者list.size=0
                        if (response.getBaseResult() == null || response.getBaseResult().toString().equals(
                                "[]"
                            )
                        ) {
                            //完成的回调所有弹窗消失
            ...
            } else if (BaseConfig.getRetSuccessList() != null) {
                //成功
                if (BaseConfig.getRetSuccessList().contains(response.getBaseCode())) {
                    //数据为空，或者list.size=0
                    if (response.getBaseResult() == null || response.getBaseResult().toString().equals(
                            "[]"
                        )
                ...
                //接口单状态码
            } else if (BaseConfig.getRetSuccess() != null) {
                //成功
                if (response.getBaseCode().equals(BaseConfig.getRetSuccess())) {
                    if (response.getBaseResult() == null || response.getBaseResult().toString().equals(
                            "[]"
                        )
                    ) {
