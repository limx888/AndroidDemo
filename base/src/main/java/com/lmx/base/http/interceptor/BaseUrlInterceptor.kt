...
open class BaseUrlInterceptor : Interceptor {
    ...
    private fun processRequest(request: Request): Request {
        ...
        if (domainName != null) {
            httpUrl = BaseConfig.getMDomainNameHub()?.get(domainName)
            //设置多baseurl的另外的baseurl,将当前的domainName进行存储，便于
        } else {
            httpUrl = BaseConfig.getMDomainNameHub()?.get(BaseConfig.getDomainName())
        }
    ...
    private fun obtainDomainNameFromHeaders(request: Request): String? {
        val headers =
            request.headers(BaseConfig.getDomainName())
        if (headers == null || headers.size == 0) return null
        require(headers.size <= 1) { "Only one " + BaseConfig.getDomainName() + " in the headers" }
        return request.header(BaseConfig.getDomainName())
    }
